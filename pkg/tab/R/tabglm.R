tabglm <- function(glmfit, latex = FALSE, xlabels = NULL, ci.beta = TRUE,
  ...
  if ((":" %in% unlist(strsplit(rownames(coef), ""))) | basic.form == TRUE) {
    ...
    tbl[2:nrow(coef), 4] <- paste(sprintf(spf, beta), " (", sprintf(spf, se),
                                  ")", sep = "")
    if (inference %in% c("wald", "wald.z")) {
      tbl[2:nrow(coef), 5] <- paste("(", sprintf(spf, beta-perc*se), ", ",
                                    sprintf(spf, beta+perc*se), ")", sep = "")
      tbl[2:nrow(coef), 7] <- paste("(", sprintf(spf, exp(beta-perc*se)), ", ",
                                    sprintf(spf, exp(beta+perc*se)), ")",
                                    sep = "")
    } else {
    ...
    for (ii in 2: length(model)) {
      ...
      if (class(model.ii)[1] != "factor" |
        ...
        tbl[tabindex, 4] <- paste(sprintf(spf, beta), " (", sprintf(spf, se),
                                  ")", sep = "")
        if (inference %in% c("wald", "wald.z")) {
          tbl[tabindex, 5] <- paste("(", sprintf(spf, beta-perc*se), ", ",
                                    sprintf(spf, beta+perc*se), ")", sep = "")
          tbl[tabindex, 7] <- paste("(", sprintf(spf, exp(beta-perc*se)), ", ",
                                    sprintf(spf, exp(beta+perc*se)), ")",
                                    sep = "")
        } else {
