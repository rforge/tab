tabmedians.svy <- function(svy, x, y, latex = FALSE, xlevels = NULL,
                           yname = "Y variable", test = "wilcoxon",
                           parenth = "iqr", text.label = NULL,
                           parenth.sep = "-", decimals = 1, p.include = TRUE,
                           p.decimals = c(2, 3), p.cuts = 0.01,
                           p.lowerbound = 0.001, p.leading0 = TRUE,
                           p.avoid1 = FALSE, n.column = FALSE,
                           n.headings = TRUE, bold.colnames = TRUE,
  ...
  if (! test %in% c("wilcoxon", "vanderWaerden", "median", "KruskalWallis")) {
    stop("For test input, please enter a possible value for the 'test' input of the svyranktest function in the survey package: 'wilcoxon', 'vanderWaerden', 'median', or 'KruskalWallis'. See documentation for tabmedians.svy and svyranktest for details.")
  }
  ...
  # Get medians and values for parentheses, and add variable name to 1st cell
  # entry to table
  ...
  medians <- svyby(~y, ~x, svy2, svyquantile, quantile = 0.5,
                   keep.var = FALSE)[, 2]
  ...
  if (parenth == "minmax") {
    ...
    tbl[1, 3] <- paste(sprintf(spf, overallmedian), " (", sprintf(spf, min(y)),
    ...
    tbl[1, 1] <- paste(yname, ", ", text.label, sep = "")
    tbl[1, 2] <- sprintf("%.0f", sum(ns))
    tbl[1, 3] <- paste(sprintf(spf, overallmedian), " (",
                       sprintf(spf, max(y) - min(y)), ")", sep = "")
    tbl[1, 4:(ncol(tbl)-1)] <- paste(sprintf(spf, medians), " (", parent, ")",
                                     sep = "")
  } else if (parenth == "q1q3") {
    parent1 <- svyby(~y, ~x, svy2, svyquantile, quantile = 0.25,
                     keep.var = FALSE)[, 2]
    parent2 <- svyby(~y, ~x, svy2, svyquantile, quantile = 0.75,
                     keep.var = FALSE)[, 2]
    parent <- paste(sprintf(spf, parent1), parenth.sep,
                    sprintf(spf, parent2), sep = "")
    if (is.null(text.label)) {
    ...
    tbl[1, 2] <- sprintf("%.0f", sum(ns))
    tbl[1, 3] <- paste(sprintf(spf, overallmedian), " (",
                       sprintf(spf, svyquantile(~y, svy2, 0.25)),
                       parenth.sep, sprintf(spf, svyquantile(~y, svy2, 0.75)),
                       ")", sep = "")
    tbl[1, 4:(ncol(tbl)-1)] <- paste(sprintf(spf, medians), " (", parent, ")",
                                     sep = "")
  } else if (parenth == "iqr") {
    ...
    tbl[1, 2] <- sprintf("%.0f", sum(ns))
    tbl[1, 3] <- paste(sprintf(spf, overallmedian), " (",
                       sprintf(spf, svyquantile(~y, svy2, 0.75) -
                                 svyquantile(~y, svy2, 0.25)), ")", sep = "")
    tbl[1, 4:(ncol(tbl)-1)] <- paste(sprintf(spf, medians), " (", parent, ")",
                                     sep = "")
