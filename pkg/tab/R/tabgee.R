tabgee <- function(geefit, latex = FALSE, xlabels = NULL, ci.beta = TRUE,
  ...
  if (rownames(coef)[1] == "(Intercept)" & intercept == TRUE) {
    beta <- coef[1, 1]
    se <- coef[1, secol]
    z <- coef[1, zcol]
    p <- pnorm(-abs(z))*2
  ...
  if ((":" %in% unlist(strsplit(rownames(coef), ""))) | basic.form == TRUE |
    ...
    tbl[2:nrow(coef), 7] <- paste("(", sprintf(spf, exp(beta-1.96*se)), ", ",
                                  sprintf(spf, exp(beta+1.96*se)), ")",
                                  sep = "")
    tbl[tabindex:nrow(coef), 8] <- formatp(p = p, cuts = p.cuts,
                                           decimals = p.decimals,
    ...
    for (ii in 1: length(xnames)) {
      ...
      if (variable.classes[ii] != "factor" ||
          (variable.classes[ii] == "factor" &
           length(unique(data[, xnames[ii]])) == 2 &
           binary.compress == TRUE))  {
        ...
        tbl[tabindex, 7] <- paste("(", sprintf(spf, exp(beta - 1.96 * se)),
                                  ", ", sprintf(spf, exp(beta + 1.96 * se)),
                                  ")", sep = "")
  ...
  # Add column names
  colnames(tbl) <- c(predictor.colname, "Clusters", "Observations", "Beta (SE)",
